...
def load_img2(img):
    ...
    for x in range(15,315):
        for y in range(15,309):
            xred = img[0][x-15:x+16,y-15:y+16]
            xgreen = img[1][x-15:x+16,y-15:y+16]
            xblue = img[2][x-15:x+16,y-15:y+16]
            yval[0][0] = img[3][x,y]
            
            xred = xred.reshape((1,961))
            xgreen = xgreen.reshape((1,961))
            xblue = xblue.reshape((1,961))
...
def binary_map(img , img_i , num , ypred , x , y):
    nrow = img[0].shape[0]
    ncol = img[0].shape[1]
...
def evaluate_lenet5(learning_rate=0.01 , lr_dec = 0.4 , n_epochs=200 , nkerns=[50,80], batch_size=468):
    ...
    trainxdirname = 'train_set/'
    trainydirname = 'train_set_anno/'
#    xdirname = '/media/rahul/1098D6BA98D69E12/academics/SEMESTER_7/BTP/anmol_dataset/colon_training_dataset/Colon_Training_Dataset/Benign/'
#    ydirname = '/media/rahul/1098D6BA98D69E12/academics/SEMESTER_7/BTP/anmol_dataset/colon_training_dataset/Colon_Training_Dataset/Benign_annotation/'
    traindataset = load_data(trainxdirname,trainydirname)
    for vdatai in range(0,36):
        imgvx,imgvy,vxgen,vygen = load_img(468,traindataset[vdatai])
        if vdatai==0:
            validx = imgvx
            validy = imgvy
        else:
    ...
    testxdirname = 'test_set/'
    testydirname = 'test_set_anno/'
    testdataset = load_data(testxdirname,testydirname)

    txgen = []
    tygen = []
    for testi in range(0,36):
        imgvx,imgvy,xgen,ygen = load_img(468,testdataset[testi])
        txgen.append(xgen)
        tygen.append(ygen)
        if testi==0:
            testx = imgvx
    ...
    # txgen = []
    # tygen = []
    # imgvx,imgvy,xgen,ygen = load_img2(dataset[30])
    # txgen.append(xgen)
    # tygen.append(ygen)
    # testx = imgvx
    # testy = imgvy 
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            for datai in range(0,36):
                imgvx,imgvy,vxgen,vygen = load_img(13,traindataset[datai])
                if datai==0:
                    trainx = imgvx
                    trainy = imgvy
                else:
...
#                                binmap_x,binmap_y,ixgen,iygen = load_img3(i+15,testdataset[im])			    	
...
#                            binary_map(testdataset[im] , im ,480*480 , ypred , binmap_xgen , binmap_ygen)
                            # binary_map(testdataset[im],im*10,480*480, yanno , binmap_xgen,binmap_ygen)
