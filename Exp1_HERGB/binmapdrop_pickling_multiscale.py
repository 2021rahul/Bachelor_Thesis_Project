...
def load_img(num,img):
    ...
    while num0<num/2 or num1<num/2:
        ...
        if(
            ...
            xh = img[3][x-25:x+26,y-25:y+26]
            xe = img[4][x-25:x+26,y-25:y+26]
            yval[0][0] = img[5][x,y]
...
def evaluate_lenet5(learning_rate=0.01 , lr_dec = 0.4 , n_epochs=200 , nkerns=[50,80], batch_size=468):
    ...
    comp1 = layer1.output.flatten(2)
    comp2 = layer0.output.flatten(2)
    layer2_input = T.concatenate((comp1,comp2),axis=1)

    layer2 = HiddenLayer(
        rng,
        input=layer2_input,
        n_in=nkerns[1] * 8 * 8 + nkerns[0]*21*21,
        n_out=468,
        activation=T.tanh,
        dropout=0.5
    )
    ...
    pickle.dump(best_params, open( "weights_multiscale.p", "wb" ) )
